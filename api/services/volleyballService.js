...
export async function getVolleyballMatches({ filter = "all" } = {}) {
  ...
  matches = pickDiverseLeagues(matches, 80).map((match) => {
...
async function fetchApiSportsMatches(apiKey) {
  const dates = Array.from({ length: 8 }, (_, index) => new Date(Date.now() + index * 24 * 60 * 60 * 1000).toISOString().slice(0, 10));
  const loaded = [];
  ...
  return loaded.map((row, index) => {
    ...
    const stats = buildSyntheticStats(homeTeam, awayTeam, row.league?.name, row.country?.name);
    return {
...
function removeFinishedMatches(matches) {
  ...
  return matches
    .filter((match) => {
      ...
      const start = new Date(match.startsAt || `${match.date}T${match.time || "00:00"}:00Z`).getTime();
...
function pickDiverseLeagues(matches, limit = 80) {
  const byLeague = new Map();
  for (const match of matches) {
    const key = `${match.country || ""} ${match.league || "Volleyball"}`.trim();
...
async function fetchVolleyballOdds(oddsKey) {
  ...
  const volleyballSports = sports.filter((sport) => String(sport.key || "").toLowerCase().includes("volleyball")).slice(0, 8);
  const allOdds = [];
  for (const sport of volleyballSports) {
    try {
      const oddsUrl = `${ODDS_BASE}/sports/${sport.key}/odds/?apiKey=${oddsKey}&regions=eu&markets=h2h,totals&oddsFormat=decimal`;
      const oddsResponse = await fetch(oddsUrl);
      if (oddsResponse.ok) {
        const rows = await oddsResponse.json();
        if (Array.isArray(rows)) allOdds.push(...rows);
      }
    } catch {
      // пропускаем один спорт, остальные пробуем дальше
    }
  }
  return allOdds;
}
...
function mergeOdds(matches, oddsRows) {
  ...
  return matches.map((match) => {
    ...
    const found = oddsRows.find((row) => {
      ...
      const direct = rowHome.includes(normalizedHome.slice(0, 8)) || rowAway.includes(normalizedAway.slice(0, 8));
      const reverse = rowHome.includes(normalizedAway.slice(0, 8)) || rowAway.includes(normalizedHome.slice(0, 8));
...
function buildDemoMatches() {
  ...
  return [
    demoMatch("volley-001", "Poland PlusLiga", "Poland", d1, "18:30", "Jastrzebski Wegiel", "Asseco Resovia", 181.5),
    demoMatch("volley-002", "Italy SuperLega", "Italy", d1, "20:00", "Trentino", "Modena", 176.5),
    demoMatch("volley-003", "Turkey Efeler Ligi", "Turkey", d1, "21:15", "Fenerbahce", "Galatasaray", 188.5),
    demoMatch("volley-004", "Nations League Women", "World", d2, "17:20", "Poland W", "Brazil W", 168.5),
    demoMatch("volley-005", "European League", "Europe", d2, "19:00", "Finland", "Netherlands", 182.5)
  ];
}
...
function buildAdvancedPrediction(match) {
  ...
  const monteCarlo = monteCarloModel(match, projectedTotal, marketLine, 800);
  ...
  return {
    ...
    model: { name: "Multi-league Market Ensemble + Monte Carlo + Set Totals v1.0", projectedTotal: round(projectedTotal, 1), rawModelTotal: round(rawModelTotal, 1), lineTotal: marketLine, totalEdge: round(totalEdge, 1), overProbability: round(ensemble.overProbability * 100, 1), underProbability: round(ensemble.underProbability * 100, 1), homeProbability: round(homeProbability * 100, 1), awayProbability: round(awayProbability * 100, 1) },
    reasons: ["Модель: Multi-league Market Ensemble + Monte Carlo + Set Totals v1.0", "Расширена загрузка матчей: теперь API смотрит 8 дней вперёд, а не только сегодня/завтра", "Тоталы сетов больше не должны быть одинаковыми: добавлен разброс по лиге, силе команд, балансу и seed матча", `Monte Carlo: ${monteCarlo.simulations} симуляций, средний тотал ${round(monteCarlo.avgTotal, 1)}`, `Линия рынка/ориентир: ${marketLine}`, `Наша модель: ${round(projectedTotal, 1)}, перевес: ${round(totalEdge, 1)}`, `Value лучшего выбора: ${round(best.valuePercent, 1)}%`, `Риск: ${risk}/10 (${risk <= 3 ? "низкий" : risk <= 6 ? "средний" : "высокий"})`]
  };
}
...
function buildSetTotals(match, projectedTotal, marketLine, ensemble, monteCarlo) {
  ...
  const leagueTempo = leagueTempoFactor(match.league, match.country);
  const balance = 1 - Math.min(Math.abs(ensemble.homeProbability - 0.5) * 1.35, 0.28);
  const expectedSets = clamp(3.25 + balance * 0.82 + Math.abs(ensemble.overProbability - 0.5) * 0.55, 3.25, 4.75);
  const baseSetLine = clamp((marketLine / expectedSets) * leagueTempo, 41.5, 53.5);
  const matchEdgePerSet = (projectedTotal - marketLine) / Math.max(expectedSets, 1);
  const rows = [
    { number: 1, label: "1-й сет", factor: 1.018 + ((seed % 11) - 5) * 0.007 },
    { number: 2, label: "2-й сет", factor: 0.998 + (((seed >> 2) % 13) - 6) * 0.008 },
    { number: 3, label: "3-й сет", factor: 0.982 + (((seed >> 4) % 17) - 8) * 0.008 }
  ];
  return rows.map((row) => {
    const rawLine = baseSetLine * row.factor;
    const line = round(toHalfPoint(clamp(rawLine, 40.5, 55.5)), 1);
    const setNoise = ((((seed + row.number * 37) % 21) - 10) / 10) * 0.45;
    const edgeWeight = row.number === 1 ? 0.95 : row.number === 2 ? 0.76 : 0.58;
    const projected = round(line + matchEdgePerSet * edgeWeight + setNoise, 1);
    const edge = round(projected - line, 1);
    const overProbability = clamp(weightedAverage([[0.5 + edge / 10.5, 0.58], [monteCarlo.overProbability, 0.20], [ensemble.overProbability, 0.22]]) * 100, 34, 66);
    const underProbability = 100 - overProbability;
    return { ...row, line, projected, edge, overProbability: round(overProbability, 1), underProbability: round(underProbability, 1), overOdd: marketOdd(null, overProbability / 100, edge), underOdd: marketOdd(null, underProbability / 100, -edge) };
  });
}

function sanitizeLines(lines, match) {
  const stats = match.stats || buildSyntheticStats(match.homeTeam, match.awayTeam, match.league, match.country);
  const fallback = buildDefaultLines(match.homeTeam, match.awayTeam, stats, lines?.total);
  return { total: isRealLine(lines?.total) ? Number(lines.total) : fallback.total, homeTotal: isRealLine(lines?.homeTotal) ? Number(lines.homeTotal) : null, awayTotal: isRealLine(lines?.awayTotal) ? Number(lines.awayTotal) : null };
}
...
function marketAdjustedTotal(rawModelTotal, marketLine, match) {
  ...
  return round(marketLine + clamp(rawEdge * 0.28, -maxMove, maxMove), 1);
}
...
function calculateTeamTotalShare(match, projectedTotal) {
  ...
  return { home: clamp(0.5 + avgDiff * 0.14 + formDiff * 0.06 + oddsDiff * 0.04, 0.45, 0.55) };
}
...
function monteCarloModel(match, projectedTotal, marketLine, simulations = 800) {
  ...
  const avgSetTotal = clamp(projectedTotal / 3.8, 39, 50.5);
  ...
  for (let i = 0; i < simulations; i += 1) {
    ...
    const total = Math.max(110, sets * avgSetTotal + normalSample(rng, 0, 7.5 + sets * 0.75));
  ...
  return { simulations, avgTotal: totalPoints / simulations, overProbability: clamp(overProbability, 0.22, 0.78), underProbability: clamp(1 - overProbability, 0.22, 0.78), homeWinProbability: clamp(homeWins / simulations, 0.30, 0.82) };
}
...
function seededRandom(seed) { let state = seed || 123456789; return () => { state = (state * 1664525 + 1013904223) >>> 0; return state / 4294967296; }; }
function teamTotalProbability(edge) { const over = clamp(0.5 + edge / 32, 0.25, 0.75); return { over, under: 1 - over }; }
function marketOdd(bookOdd, probability, edge = 0) { if (Number(bookOdd) >= 1.4) return round(Number(bookOdd), 2); const fair = probability > 0 ? 1 / probability : 1.9; const margin = 1.03 + Math.min(Math.abs(edge), 9) * 0.004; return clamp(round(fair * margin, 2), 1.4, 3.2); }
function formModelScore(match) { const diff = match.stats.homeForm - match.stats.awayForm; return { homeProbability: clamp(0.5 + diff * 0.48, 0.34, 0.76) }; }
function oddsModelScore(match) { const home = implied(match.odds.home); const away = implied(match.odds.away); const total = home + away || 1; return { homeProbability: clamp(home / total, 0.32, 0.78) }; }
function powerRatingModel(match, projectedTotal) { const homePower = match.stats.homeAvgTotal * 0.30 + match.stats.homeForm * 100 * 0.70; const awayPower = match.stats.awayAvgTotal * 0.30 + match.stats.awayForm * 100 * 0.70; const diff = (homePower - awayPower) / Math.max(projectedTotal, 1); return { homeProbability: clamp(0.5 + diff, 0.34, 0.76) }; }
function ensembleModel({ poisson, formModel, oddsModel, powerModel, setsModel, monteCarlo }) { const homeProbability = weightedAverage([[formModel.homeProbability, 0.24], [oddsModel.homeProbability, 0.34], [powerModel.homeProbability, 0.22], [monteCarlo.homeWinProbability, 0.20]]); const overProbability = weightedAverage([[poisson.overProbability, 0.34], [setsModel.overProbability, 0.22], [monteCarlo.overProbability, 0.34], [clamp(0.5 + (powerModel.homeProbability - 0.5) * 0.10, 0.46, 0.54), 0.10]]); return { homeProbability: clamp(homeProbability, 0.32, 0.80), overProbability: clamp(overProbability, 0.26, 0.74), underProbability: clamp(1 - overProbability, 0.26, 0.74) }; }
function buildCandidate(type, selection, probability, odd, edge) { const valuePercent = (probability * Number(odd || 0) - 1) * 100; const edgeBonus = Math.min(Math.abs(edge), 8) * 0.010; const valueBonus = Math.max(-0.14, Math.min(valuePercent / 100, 0.24)); return { type, selection, probability, odd: round(odd, 2), edge, valuePercent, score: probability * 0.66 + valueBonus * 1.45 + edgeBonus }; }
function buildSyntheticStats(homeTeam, awayTeam, league = "", country = "") { const homeSeed = seedNumber(`${homeTeam}-${league}`); const awaySeed = seedNumber(`${awayTeam}-${country}`); const tempo = leagueTempoFactor(league, country); const base = (174 + ((homeSeed + awaySeed) % 14)) * tempo; return { homeAvgTotal: base + (homeSeed % 7), awayAvgTotal: base + (awaySeed % 7), homeForm: 0.45 + (homeSeed % 35) / 100, awayForm: 0.45 + (awaySeed % 35) / 100, h2hAvgTotal: base + ((homeSeed - awaySeed) % 9) }; }
function leagueTempoFactor(league = "", country = "") { const text = `${league} ${country}`.toLowerCase(); if (text.includes("women") || text.includes("жен")) return 0.985; if (text.includes("plusliga") || text.includes("superlega")) return 1.035; if (text.includes("nations")) return 1.0; if (text.includes("european")) return 0.995; return 1 + ((seedNumber(text) % 9) - 4) * 0.006; }
function buildDefaultLines(homeTeam, awayTeam, stats = buildSyntheticStats(homeTeam, awayTeam), forcedTotal = null) { const raw = average([stats.homeAvgTotal, stats.awayAvgTotal, stats.h2hAvgTotal]); const total = round(Number.isFinite(Number(forcedTotal)) && Number(forcedTotal) > 80 ? Number(forcedTotal) : clamp(raw - 12.5, 160.5, 198.5), 1); return { total, homeTotal: null, awayTotal: null }; }
function recalcTeamLines(lines, match) { const stats = match.stats || buildSyntheticStats(match.homeTeam, match.awayTeam, match.league, match.country); return { ...buildDefaultLines(match.homeTeam, match.awayTeam, stats, lines.total), homeTotal: null, awayTotal: null }; }
function defaultOdds(homeTeam = "", awayTeam = "") { const homeSeed = seedNumber(homeTeam); const awaySeed = seedNumber(awayTeam); return { home: round(1.58 + (homeSeed % 42) / 100, 2), away: round(1.64 + (awaySeed % 58) / 100, 2), over: round(1.72 + ((homeSeed + awaySeed) % 30) / 100, 2), under: round(1.72 + ((homeSeed * 3 + awaySeed) % 30) / 100, 2) }; }
function calculateRisk(probability, edge, valuePercent) { const risk = 9.0 - probability * 4.8 - Math.min(edge, 8) * 0.13 - Math.max(-6, valuePercent) * 0.035; return clamp(round(risk, 1), 1, 10); }
